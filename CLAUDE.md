...
- `GameManager` — fires `OnGameWon` / `OnGameLost`. `InitializeGame()` is called by `LevelManager.LoadMap` (no longer by `GameManager.Start`).
  ...
  - **Win 2 (robust per-frame):** `BallCount == 0` AND gameplay has started (subscribes to `BallSpawner.OnIntroComplete` to set a `gameplayStarted` flag). Catches icicles and any other removal path that doesn't fire `OnChainCleared`.
  - **Win 3 (retreat):** `BallCount > 0` AND `!HasVisibleBalls()` AND intro not playing (all balls retreated into the hole).
