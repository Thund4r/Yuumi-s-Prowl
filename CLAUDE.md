...
- **Color weighting:** `BallColorUtils.GetWeightedRandomColor` (run-avoidance, used by `BallSpawner`) and `PickWeightedColor` (used by `ProjectileSpawner`) bias colour choice by `RuntimeStats.ColorWeights`. So projectile colours lean the same way as the chain.
