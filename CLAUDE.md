...
- `MatchProcessor` — owns the segmented match pipeline:
  ...
  - **Cascade detection is consistent across every gap-closure source.** `ProcessPierceAftermath`, `ProcessHammerAftermath`, and `BombAftermathCoroutine` all follow the same two-phase pattern: Phase 1 scans for and resolves any *immediate* matches at the removal site; Phase 2 calls `WatchGapCloseCascade()` which registers a `MatchSequenceState` keyed by the front segment id and waits for back-most via `WaitForBackMost`. While the front segment is closing the gap, `OnSegmentsMergedHandler` fires cascade detection on every merge seam (sharing the same sequence). Without this Phase 2 registration, `OnSegmentsMergedHandler` would bail (`if (!sequencesById.TryGetValue(segId, out var seq)) return;`) and Pierce/Bomb/Explosion gap-closes would silently skip cascade checks.
...
- **Fire-and-forget homing** (Step 8): `Projectile.SetHoming(strict, loose, range)` is set by `ProjectileSpawner` (at spawn and refreshed at launch) from `RuntimeStats.HomingStrictEnabled/HomingLooseEnabled/HomingRange`. **Lock-and-commit:** once a valid same-color ball comes within `HomingRange`, the projectile locks onto that `Ball` *object* (`homingLock`) and commits — it follows the ball's live position (so chain movement / gap-closing can't shake it) and **passes through every other ball and obstacle** until it reaches its locked ball. A proximity check (`homingArrivalDistance`) inserts directly if it gets close, preventing orbit-misses. The lock drops only if the ball is destroyed/leaves the chain (then it re-acquires) or the player holds **right-mouse** (manual steer — collision behaves normally while held). **Strict** mode only locks onto a ball with a same-color neighbor (guarantees a 3+ match); **Loose** mode accepts any same-color ball, subsumes strict, and enables multi-fire in `ProjectileSpawner`. Hammer/power-up balls are never targeted.
...
- **Synergy tagging:** an `UpgradeDefinition` flagged `IsColorSynergy` belongs to `TargetColor`'s synergy. Such upgrades get a coloured card background (`BallColorUtils.GetSynergyBackgroundColor`) in all three card UIs, and count toward that colour's synergy total via `RunState.CountColorSynergyUpgrades(color)`.
- **Count-based scaling:** `RunManager.RecomputeSynergyStats()` (called as each gameplay floor loads) derives stats from synergy-upgrade counts: `ExplosionRadius = RunConfig.baseExplosionRadius + redCount × explosionRadiusPerRedUpgrade`, and each colour's spawn weight = `1.0 + count × RunConfig.colorWeightPerSynergyUpgrade`. Neither is an upgrade card — they scale purely with how many synergy upgrades of that colour you own.
- **Color weighting:** `BallColorUtils.GetWeightedRandomColor` (run-avoidance) biases `BallSpawner`'s tail-ball picks by `RuntimeStats.ColorWeights`. `ProjectileSpawner.GetNextColor` uses the **same `ColorWeights`** as the ball spawner, but **zeroes out any colour that isn't currently present in the chain**. So projectile colours follow the same intended distribution as the chain, and a colour fully cleared from the chain stops generating projectiles until tail spawns put it back. Falls back to uniform pick only when every colour is absent (empty chain between levels).
- **Color-match gold:** `RunManager.HandleBallsDestroyed` grants `RuntimeStats.GetColorMatchGold(color)` each time a match of that colour is destroyed. `ColorMatchGold` is still a per-card upgrade (`UpgradeStat.ColorMatchGold` + `TargetColor`).
...
- **Purple rage synergy:** `RageMeter` (scene component) is gated by `RuntimeStats.RageEnabled` — set by the **`RageUnlock`** anchor upgrade (mirrors how `RedMatchExplosion` enables red explosions). Without the anchor, no rage. With it, the meter subscribes to `MatchProcessor.OnBallsDestroyed` and **only purple matches build the meter** — `HandleBallsDestroyed` filters by `color != BallColor.Purple`, so purple synergy is self-reinforcing (more purple upgrades → more purple balls spawned via `ColorWeights` → faster rage charging). Each destroyed purple ball adds `RunConfig.rageGainPerBall + RuntimeStats.RageBuildupBonus` rage. **Rage no longer auto-activates** — when the bar fills, the meter goes into a `IsReady` state and fires `OnRageReady`. The player presses `RageMeter.activationKey` (default Space) OR taps `RageMeterUI.activationButton` to fire `TryActivateRage()`. Once activated, rage runs for `RunConfig.rageDuration + RuntimeStats.RageDurationBonus` seconds, sets `RuntimeStats.HomingStrictEnabled` (and `HomingLooseEnabled` if total purple synergy count ≥ `RunConfig.rageLoosePurpleCount` — granting multi-fire too), then expires and resets to 0. **Round-end refund:** if the round ends (GameManager.OnGameWon/OnGameLost) while rage is active, the meter cancels immediately and refunds the *unused timer fraction* back to `CurrentRage` — e.g. ending at 60% time remaining starts the next round at 60% rage. `RageMeter` subscribes to GameManager directly for this; the GameManager reference is optional but the refund won't fire without it. `RageMeterUI` shows an activation prompt label (TMP_Text, optional) above the bar while ready — text format is `promptFormat` with `{0}` filled from `ActivationKey`. The bar also tints differently for filling / ready / active states. The foreground Image must be `Image Type = Filled`. If the optional `visibleRoot` is used, do **not** point it at `RageMeterUI`'s own GameObject — the script guards against that (would self-disable and never re-enable on unlock), but it's still a misconfig. Purple stat upgrades: `RageBuildupRate` (faster fill), `RageDuration` (longer active window), `FireRate` (shorter projectile cooldown — always-on), plus the existing `HomingRange`. The `HomingStrict` / `HomingLoose` upgrade stats are **deprecated** — rage now sets those flags dynamically.
- **Blue ice-patches synergy:** `IceSynergy` (scene component) gated by `RuntimeStats.IcePatchesEnabled` — set by the **`IcePatches`** anchor upgrade. With it active: blue matches spawn a world-space ice patch (`RunConfig.icePatchRadius`, `icePatchDuration`) at the match centroid. Per-frame, every ball inside the disc gets a frost stack on first contact; re-entry can re-stack only after `icePatchReentryCooldown` since exit (default 2s — prevents per-frame stack spam). At `iceFreezeStackThreshold` stacks (default 3), the ball becomes frozen and its stacks zero out. **Frozen balls still move with the chain** — they're just visually icy and tagged. When a frozen ball is removed by *anything* (match, bomb, pierce, red explosion, another icicle), `BallChainManager` fires `OnFrozenBallDestroyed(worldPos)`. `IceSynergy` listens, picks a random ball not currently targeted by another icicle, and spawns an `Icicle` from the destruction site that homes to the target at `RunConfig.icicleSpeed` and destroys it on contact (`icicleArrivalDistance`). Icicles use `MatchProcessor.ProcessPierceAftermath` so cascade detection runs the same as bomb/pierce. Chain reactions: icicle destroys frozen ball → spawns another icicle. `IceSynergy` keeps a `HashSet<Ball> targetedBalls` so spread is enforced. Per-patch state lives in `IcePatch.perBallState` (`Dictionary<BallNode, IcePatchBallState>`); both pool sizes (patch visuals, icicles) are serialized. Frost-stack and frozen visuals live entirely on the **`frozenOverlay` child** wired into the Ball prefab. The overlay's material alpha is driven by the stack count (`frostAlphaPerStack`); on freeze the overlay snaps to full opacity AND its material colour swaps to the prefab's authored `frozenColor` field (Ball caches each overlay material's base RGB at Awake so de-freeze restores the original tint). Overlay scale is **not** stack-driven — author the overlay prefab at its final on-screen size. The ball's base material colour is never tinted; readability is preserved through the translucent overlay. **Overlay materials must use a transparency-capable shader** (the script instances them on Awake and drives `material.color` at runtime).
- **Blue follow-up upgrades** (all gated by `IcePatches`):
  - **`CryoBurst`** (flag): on every blue match, `IceSynergy.HandleMatchVisual` defers one frame then runs `CryoBurstRoutine` — a coroutine that grows a world-space ring from 0 → `RunConfig.cryoBurstRadius` over `cryoBurstDuration`, calling `Physics.OverlapSphere` each frame with the current radius. Hit balls get +1 frost stack via the shared stack-application path; a `HashSet<Ball>` dedupes so each ball is hit at most once per burst. Each hit ball also gets a `Ball.FlashFrost()` shimmer for player feedback.
  - **`CryoBurstChain`** (flag, prereq `CryoBurst`): extends Cryo Burst — `HandleFrozenBallDestroyed` fires an additional cryo burst at the destruction position alongside spawning the icicle. Capped at **one burst per frame total** (`lastBurstFrame` tracks both match-triggered and cascade-triggered bursts), so a single destruction event that kills multiple frozen balls — a blue match, a red match, a bomb, a pierce, a red explosion — fires one burst, not N. Chain reactions across frames still propagate normally (each icicle hit on a separate frame is its own burst).
  - **`FreezeTheHunted`** (flag): `PickRandomUntargetedBall` collects frozen-and-untargeted balls into a side list; if non-empty, picks from that list first, otherwise falls back to the standard random pick.
...
3. Update `LevelData.colorCount` range if needed (`[Range(1, 5)]` currently caps at 5).
...
2. Set **Upgrade Id** (a stable unique string — required for meta upgrades, used as the save key), **Upgrade Name**, **Description**, optional **Icon**, and **Modifier Value**.
   ...
   - *Raw* stats (RotationSpeed, ChargePerBall, GoldPerCascade, BallSpeedReduction, DraftReroll) — raw delta per rank (e.g. RotationSpeed `+50` deg/s per rank).
   - *Note:* `BombRadius` is deprecated as an upgrade stat — explosion radius is count-scaled from red synergy upgrades (`RunConfig.baseExplosionRadius` + `explosionRadiusPerRedUpgrade` × red-upgrade-count). Don't author `BombRadius` cards.
...
- MainMenu Settings/Options menu (Play and Quit buttons are wired, but no settings screen yet).
