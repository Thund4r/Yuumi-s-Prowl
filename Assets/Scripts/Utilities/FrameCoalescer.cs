...
namespace YuumisProwl.Utilities
{
    ...
    public class FrameCoalescer
    {
        ...
        private readonly System.Action<Vector3, int> onResolve; // (clusterCentroid, memberCount)
        ...
        public FrameCoalescer(MonoBehaviour owner, float mergeRadius,
                              System.Action<Vector3, int> onResolve)
        {
            this.owner = owner;
        ...
        private IEnumerator ResolveNextFrame()
        {
            ...
            for (int i = 0; i < pending.Count; i++)
            {
                ...
                if (hit >= 0) { sums[hit] += p; counts[hit]++; }
            ...
            for (int c = 0; c < sums.Count; c++)
                onResolve?.Invoke(sums[c] / counts[c], counts[c]);
        }
    }
}
